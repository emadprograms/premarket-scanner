...
- **Decision: Price Ladder Sidebar**: Converted from below-chart section into a collapsible sidebar overlay on the left edge of the chart. Collapsed state: thin tab with ChevronRight icon at vertical center. Expanded state: 220px frosted-glass sidebar (`bg-zinc-950/95 backdrop-blur-md`) slides in with 300ms CSS `translate-x` transition. Tab animates to `left-[220px]` when open. Compact entry layout for sidebar fit.
## 9. The Data Pipeline
