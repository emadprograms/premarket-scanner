...
- **Decision: Archive Sidebar Trend Bias**: Changed the company list in the Archive Room sidebar to display the **Trend Bias** (multi-day narrative) instead of the Setup Bias (tactical next-day plan). This ensures the high-level directional bias is visible at a glance when browsing historical records. Updated the extraction logic to parse from the `confidence` field and implemented color-coded arrows: Emerald (Pure Bull), Rose (Pure Bear), and Grey (Neutrals/Leans).
...
- **Decision: High Contrast Mode**: Toggleable via HC pill button in chart controls bar (next to VP) and gear popover. When ON: light gray background (`#d4d4d4`), black/white candles (up=white with black border, down=black), subtle gridlines (`rgba(0,0,0,0.06)`), dark text (`#27272a`), solid dark crosshair (`rgba(0,0,0,0.8)`), and highly visible S/R lines (`0.7` opacity). Volume Profile switches to grey (POC=dark grey `rgba(50,50,50,0.6)`, bars=`rgba(80,80,80,opacity)`). Triggers full chart + VP rebuild via `highContrast` in both useEffect dep arrays.
## 9. The Data Pipeline
