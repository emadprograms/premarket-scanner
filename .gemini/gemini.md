...
- **Decision: Widescreen Layout**: Removed `max-w-7xl` cap from the main page layout and widened the chart popup modal to `max-w-[95vw]`. Card grid uses `repeat(auto-fill, minmax(280px, 1fr))` so columns scale dynamically with screen width — no hardcoded column limits.
- **Decision: ETH/RTH Session Toggle**: Added an ETH/RTH pill toggle next to the timeframe pills. Default is ETH. RTH filters bars outside 9:30 AM – 4:00 PM ET using `hourCycle: 'h23'` for DST-safe offset. Skipped for `DAY` resolution. Falls back to ETH if filter removes all bars. Chart uses `fitContent()` instead of `setVisibleLogicalRange` so candle density adapts naturally on resize.
## 9. The Data Pipeline
