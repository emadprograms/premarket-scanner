...
- **Decision: High Contrast Mode**: Toggleable via HC pill button in chart controls bar (next to VP) and gear popover. When ON: light gray background (`#d4d4d4`), black/white candles (up=white with black border, down=black), visible gridlines (`rgba(0,0,0,0.12)`), dark text (`#27272a`), dark crosshair. Both toggles sync — gear popover sets session default, per-chart HC button overrides locally. Triggers full chart rebuild via `highContrast` in useEffect dep array.
## 9. The Data Pipeline
