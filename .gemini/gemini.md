...
- **Decision: Robust Price Level Regex**: Refactored `S_Levels` and `R_Levels` extraction in `ChartPlanView.tsx` and `database.py`. The regex now uses a non-greedy `[\s\S]*?` capture with an lookahead/alternation to handle both multi-line bracketed AI output (common in specific GOOGL cards) and single-line non-bracketed fallback text.
