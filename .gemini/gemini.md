...
- **Decision: Chart Popup Enhancements**: Upgraded `ChartPlanView.tsx` with: (1) Timeframe pills (1m, 5m, 30m, 1H, 4H, 1D) with data-source-aware lookback limits, (2) Expanded price ladder by default, (3) Modal widened to `max-w-[95vw]` and chart height `500px`, (4) Fixed `barSpacing: 12` with `scrollToRealTime()` for consistent candle width across all timeframes/sessions, (5) Position size displayed as a compact inline badge in the header row (center, between timeframe pills and data source toggle).
...
- **Decision: Fullscreen Chart**: Browser Fullscreen API (`requestFullscreen`/`exitFullscreen`). Maximize2/Minimize2 icon in top-right. `chartWrapperRef` wraps both controls bar and chart so controls are visible in fullscreen. Uses `flex flex-col h-screen` in fullscreen: controls at top, chart `flex-1` fills remaining height. Resize handler updates both width and height via `chart.applyOptions()`. Listens to `fullscreenchange` with 100ms delay for CSS layout recalculation. Control bar gets `px-4 pt-3 pb-2` padding in fullscreen.
- **Decision: Price Ladder Sidebar**: Collapsible left-side sidebar overlay on the chart. ChevronRight toggle tab at `top-2`. Expanded: 260px wide, `bg-zinc-950/95 backdrop-blur-md`, content-fit height (no blank space below entries). No header label — entries start immediately. Price text `text-xs`, labels `text-[11px]`. Compact gaps/padding for tight fit. Slide animation via `translate-x` with 300ms transition.
- **Decision: Chart Session Defaults**: Gear icon (Settings) in the Shell header opens a popover with two sections: (1) **Chart Defaults** — data source (Capital/Yahoo), timeframe (1m–1D), session (ETH/RTH), VP toggle, HC toggle. (2) **Position Sizing** — Base Capital ($, +/- $500 steppers) and Risk Tolerance (%, +/- 0.1 steppers). Stored in `MissionSettings` context. `ChartPlanView` reads chart defaults via `useMission()` as initial state values.
- **Decision: High Contrast Mode**: Toggleable via HC pill button in chart controls bar (next to VP) and gear popover. When ON: light gray background (`#d4d4d4`), black/white candles (up=white with black border, down=black), subtle gridlines (`rgba(0,0,0,0.06)`), dark text (`#27272a`), dark crosshair. Volume Profile switches to grey (POC=dark grey `rgba(50,50,50,0.6)`, bars=`rgba(80,80,80,opacity)`). Triggers full chart + VP rebuild via `highContrast` in both useEffect dep arrays.
- **Decision: Horizontal Ray Drawing**: Toggle draw mode via Alt+J or Pencil+RAY button in controls bar. In draw mode, cursor becomes crosshair; clicking the chart places a horizontal price line at the crosshair's close price using `series.createPriceLine()`. Lines are cyan dashed (`rgba(6,182,212,0.7)`) in normal mode, dark grey (`rgba(50,50,50,0.8)`) in HC mode. Trash button appears when rays exist to clear all. Uses `subscribeCrosshairMove` to track price under cursor, `drawModeRef` for stable click handler reference. Escape key exits draw mode. Rays are session-only (lost on chart rebuild).
## 9. The Data Pipeline
