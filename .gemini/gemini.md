...
- **Decision: Chart-First Modal**: Clicking a card shows a lightweight-charts candlestick chart with Plan A/B plotted as horizontal price lines (solid=Plan A, dashed=Plan B, dotted=live price). Plan level cards below show price, S/R nature, description, and distance from live price. A prominent "Read Screener Briefing" button leads to the text briefing. Full card is accessible from there. The flow is: Chart → Briefing → Full Card.
...
- **Decision: Chart Loading UX**: Chart modal shows a pulsing heartbeat SVG icon with a violet shimmer bar while fetching Capital.com data. Falls back to synthetic candles with a warning if unavailable.
