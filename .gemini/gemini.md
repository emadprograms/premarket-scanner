...
- **Decision: Chart Session Defaults**: Gear icon (Settings) in the Shell header opens a popover with session-wide chart defaults: data source (Capital/Yahoo), timeframe (1m–1D), session (ETH/RTH), VP toggle (ON/OFF). Stored as `chartDefaults` in `MissionSettings` context. `ChartPlanView` reads defaults via `useMission()` as initial state values. Changed defaults apply to all newly opened cards for the session.
## 9. The Data Pipeline
