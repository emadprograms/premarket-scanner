...
- **Decision: Widescreen Layout & Columns**: Removed `max-w-7xl` cap from the main page layout and widened the chart popup modal to `max-w-[95vw]` so the dashboard and charts utilize the full width of ultrawide monitors. Aggressively scaled the CSS Grid breakpoints (`min-[1600px]:grid-cols-6`, `min-[1750px]:grid-cols-7`) so widescreen users get up to 7 cards per row rather than excessively wide single cards.
- **Decision: ETH/RTH Session Toggle & Dynamic Candles**: Added an ETH/RTH pill toggle next to the timeframe pills in the chart popup. Default is ETH. RTH filters out bars outside 9:30 AM – 4:00 PM ET using a DST-aware UTC-to-ET offset. Extended the dynamic bar calculation to factor in the active session: `visibleBars` is calculated as `chartWidth / 7` for RTH (showing more bars = thinner candles to compensate for lower total bar count per day) and `chartWidth / 18` for ETH (showing fewer bars = thicker candles to prevent squishing).
## 9. The Data Pipeline
