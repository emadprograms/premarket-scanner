...
# --- LOCAL IMPORT CHECK ---
# We assume key_manager.py is in the SAME folder as this script.
try:
    from key_manager import KeyManager
except ImportError:
    st.error("❌ CRITICAL MISSING FILE: 'key_manager.py' was not found in this directory.")
    st.info("Please ensure key_manager.py is in the same folder as this script.")
    st.stop()
...
# API Base URL (Model will be appended dynamically)
...
# ==============================================================================
# 2. GLOBAL INITIALIZATION (SECRETS & KEY MANAGER)
# ==============================================================================
...
# ==============================================================================
# 4. DATABASE UTILITIES (CACHED & ROBUST)
# ==============================================================================

@st.cache_resource(show_spinner="Connecting to Headquarters...")
def get_db_connection():
    """
    Establishes a cached connection to Turso using the HTTPS URL.
    """
...
def init_db_schema(client, logger: AppLogger):
    try:
        ...
        logger.log(f"DB Schema Verify Error: {e}")
...
def process_session_data_to_summary(ticker: str, df: pd.DataFrame, live_price: float, logger: AppLogger) -> dict:
    ...
    if df is None or df.empty:
        result["summary_text"] = f"Data Summary: {ticker} (No Session Bars. Price: ${live_price:.2f})"
        return result
    ...
    if not pd.isna(pm_high) and not pd.isna(pm_low):
        ...
        if pos > 0.75: trend_msg = "Trending High"
        elif pos < 0.25: trend_msg = "Trending Low"
...
# ==============================================================================
# 6. AI SERVICES (KEY MANAGER INTEGRATED)
# ==============================================================================
...
def call_gemini_with_rotation(
    ...
    """
    Robust API Loop that rotates keys, checks quotas, and reports health.
    Uses KEY_MANAGER_INSTANCE.
    """
    ...
    for i in range(max_retries):
        ...
        try:
            ...
            # 2. EXECUTE
            # Dynamically build the URL based on the selected model
...
def main():
    ...
    with st.sidebar:
        ...
        if KEY_MANAGER_INSTANCE:
            st.success("✅ Key Manager: Online")
        else:
            st.error("❌ Key Manager: Offline")
        ...
        selected_model = st.selectbox(
            ...
            AVAILABLE_MODELS, 
            index=0,
            help="Select the AI model to power the analysis."
        )
    ...
    with tab1:
        ...
        if st.session_state.glassbox_etf_data:
            ...
            etf_placeholder.info("Waiting for market data scan...")
        ...
        if st.button("Generate Economy Card (Step 0)", key="btn_step0", type="primary"):
            ...
            with st.status(f"Running Macro Scan ({mode})...", expanded=True) as status:
                ...
                # --- REPLACED CALL: Use call_gemini_with_rotation ---
                resp, error_msg = call_gemini_with_rotation(
                    prompt=prompt,
                    system_prompt=system_prompt,
                    logger=logger,
                    model_name=selected_model # <--- Model passed here
                )
        ...
        if st.button("Run Synthesis", type="primary"):
            ...
            with st.spinner(f"Head Trader Categorizing ({selected_model})..."):
                ...
                briefing, error_msg = call_gemini_with_rotation(
                    prompt=prompt,
                    system_prompt="You are an elite Head Trader.",
                    logger=logger,
                    model_name=selected_model # <--- Model passed here
                )
