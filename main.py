...
try:
    ...
    st.error(f"Critical Error loading secrets: {e}. Check gemini and turso keys.")
    st.stop()
...
def main():
    ...
    with st.sidebar:
        ...
        # CRITICAL: SQLite string comparison fails if we mix ' ' and 'T' separators.
        # '2025-11-06 23:00:00' (Space) is lexically LESS than '2025-11-06T13:00:00' (T).
        # This caused "future" data to leak in.
        # FIX: We format the cutoff string to match the DB's likely format (Space separator).
    ...
    with tab2:
        ...
        if st.button("Run Synthesis", type="primary"):
            ...
            with st.spinner("Head Trader Categorizing..."):
                prompt = (
                    "[INPUTS]\n"
                ...
                briefing, error_msg = call_gemini_api(
                    prompt,
                    API_KEYS,
